...
this.toShow=a([]);this.toHide=a([]);this.currentElements=a([]);},prepareForm:function(){this.reset();this.toHide=this.errors().add(this.containers);},prepareElement:function(b){this.reset();this.toHide=this.errorsFor(b);},check:function(i){i=this.clean(i);if(this.checkable(i)){i=this.findByName(i.name).not(this.settings.ignore)[0];
}var g=a(i).rules();var b=false;for(var h in g){var f={method:h,parameters:g[h]};try{var d=a.validator.methods[h].call(this,i.value.replace(/\r/g,""),i,f.parameters);if(d=="dependency-mismatch"){b=true;continue;}b=false;if(d=="pending"){this.toHide=this.toHide.not(this.errorsFor(i));return;}if(!d){this.formatAndAdd(i,f);
return false;}}catch(c){this.settings.debug&&window.console&&console.log("exception occured when checking element "+i.id+", check the '"+f.method+"' method",c);throw c;}}if(b){return;}if(this.objectLength(g)){this.successList.push(i);}return true;},customMetaMessage:function(b,d){if(!a.metadata){return;
...
return this.optional(b)||c.match(new RegExp(".("+d+")$","i"));},equalTo:function(c,b,e){var d=a(e).unbind(".validate-equalTo").bind("blur.validate-equalTo",function(){a(b).valid();});return c==d.val();}}});a.format=a.validator.format;})(jQuery);(function(a){var b={};if(a.ajaxPrefilter){a.ajaxPrefilter(function(d,g,e){var f=d.port;
if(d.mode=="abort"){if(b[f]){b[f].abort();}b[f]=e;}});}else{var c=a.ajax;a.ajax=function(d){var e=("mode" in d?d:a.ajaxSettings).mode,f=("port" in d?d:a.ajaxSettings).port;if(e=="abort"){if(b[f]){b[f].abort();}return(b[f]=c.apply(this,arguments));}return c.apply(this,arguments);};}})(jQuery);(function(a){if(!jQuery.event.special.focusin&&!jQuery.event.special.focusout&&document.addEventListener){a.each({focus:"focusin",blur:"focusout"},function(c,b){a.event.special[b]={setup:function(){this.addEventListener(c,d,true);
},teardown:function(){this.removeEventListener(c,d,true);},handler:function(e){arguments[0]=a.event.fix(e);arguments[0].type=b;return a.event.handle.apply(this,arguments);}};function d(e){e=a.event.fix(e);e.type=b;return a.event.handle.call(this,e);}});}a.extend(a.fn,{validateDelegate:function(d,c,b){return this.bind(c,function(e){var f=a(e.target);
if(f.is(d)){return b.apply(f,arguments);}});}});})(jQuery);jQuery.validator.addMethod("tableName",function(c,d){return this.optional(d)||/^[\w]+$/.test(c);},"只能包括英文字母、数字和下划线");jQuery.validator.addMethod("isMobile",function(f,e){var d=f.length;return this.optional(e)||(d==11&&/^(((1[0-9]{1}))+\d{9})$/.test(f));
},"请正确填写您的手机号码");jQuery.validator.addMethod("isPhone",function(f,d){var e=/^(\d{3,4}-?)?\d{7,9}$/g;return this.optional(d)||(e.test(f));},"请正确填写您的电话号码");jQuery.validator.addMethod("isZipCode",function(f,d){var e=/^[0-9]{6}$/;return this.optional(d)||(e.test(f));},"请正确填写您的邮政编码");